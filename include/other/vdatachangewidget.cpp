  ...
  ui(new Ui::VDataChangeWidget)
{
  ...
  QStringList headerLables; headerLables << "Enabled" << "Log" << "Pattern" << "Syntax" << "Case" << "Minimal" << "Find Only" << "Replace";

  ui->treeWidget->setHeaderLabels(headerLables);
  ui->treeWidget->setColumnWidth(ENABLED_IDX,   50);
  ...
  ui->treeWidget->setColumnWidth(FIND_ONLY_IDX, 30);

  ui->treeWidget->header()->setSectionResizeMode(ENABLED_IDX,   QHeaderView::Interactive);
  ui->treeWidget->header()->setSectionResizeMode(LOG_IDX,       QHeaderView::Interactive);
  ui->treeWidget->header()->setSectionResizeMode(PATTERN_IDX,   QHeaderView::Stretch);
  ui->treeWidget->header()->setSectionResizeMode(SYNTAX_IDX,    QHeaderView::Interactive);
  ui->treeWidget->header()->setSectionResizeMode(CS_IDX,        QHeaderView::Interactive);
  ui->treeWidget->header()->setSectionResizeMode(MINIMAL_IDX,   QHeaderView::Interactive);
  ui->treeWidget->header()->setSectionResizeMode(FIND_ONLY_IDX, QHeaderView::Interactive);
...
void operator << (VDataChangeItem& item, QTreeWidgetItem& treeWidgetItem)
{
  ...
  item.findOnly = treeWidgetItem.checkState(VDataChangeWidget::FIND_ONLY_IDX) == Qt::Checked;
...
void operator << (QTreeWidgetItem& treeWidgetItem, VDataChangeItem& item)
{
  ...
  treeWidgetItem.setCheckState(VDataChangeWidget::FIND_ONLY_IDX, item.findOnly ? Qt::Checked : Qt::Unchecked);
